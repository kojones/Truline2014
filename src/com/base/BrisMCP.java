...
public class BrisMCP
{
 ...
 public boolean load(String file, String filename)
 {
  ...
  if (!parseMcp(file, filename))
   return false;
  return true;
 }
 public String[] namesMcp = {
   //
   // MAX
  ...
  */
 public boolean parseMcp(String file, String filename)
 {
  boolean status = true;
  String buffer;
  ...
  try {
   in = openFile(file, ".MCP");
  } catch (Exception e) {
   int i = file.indexOf('.');
   if (i > 0)
    file = file.substring(0, i);
   Truline.println("Could not open file - " + file + ".MCP\n   "
     + e.getMessage());
   Log
     .print("Could not open file - " + file + ".MCP\n   " + e.getMessage());
   return false;
  }
  Truline.println("Parsing " + filename + ".MCP");
  try {
   ...
   Truline.println("Could not read file - " + filename + ".mcp\n   "
     + e.getMessage());
   Log
     .print("Could not read file - " + filename + ".mcp\n   " + e.getMessage());
   return false;
  }
  String value;
  int fld = 0;
   ...
   try {
    ...
    switch (c) {
     ...
     case StreamTokenizer.TT_WORD:
      fld_in++;
      if (fld < namesMcp.length) {
       name = namesMcp[fld].trim();
       if (parser.sval == null)
        nameVal = "";
       else
        nameVal = parser.sval;
        ...
        if (name == "TRAINER") {
         ...
         if (w_raceNo == 0 || w_raceNo != Lib.atoi(value)) {
          if (race.m_props.size() > 0) {
            ...
            * = true; fld = 1; while (moreProp) { name = namesMcp[fld].trim();
      ...
      if (nameVal.substring(0, 4) == "Near" || nameVal.lastIndexOf(",") != -1)
       ...
       break;
     case ',':
      if (fld < namesMcp.length && namesMcp[fld].startsWith("WORK")) {
       j++;
       if (j >= 12) {
        j = 0;
        fld++;
       ...
       if (j >= 10) {
        if (running) {
         if (namesMcp[fld].equals("FRONTBANDAGE"))
          pastPerf = false;
         j = 0;
         fld++;
        }
    ...
    // an error condition...
    System.out.println("Error in " + filename + ".MCP: line=" + parser.lineno()
