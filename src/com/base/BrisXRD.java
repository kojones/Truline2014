...
public class BrisXRD
{
 ...
 public boolean parseXrd(String file, String filename)
 {
  ...
  try {
   ...
   Truline.println("Could not open file - " + filename + ".xrd\n   "
     + e.getMessage());
   Log
     .print("Could not open file - " + filename + ".xrd\n   " + e.getMessage());
   return false;
  }
    ...
    switch (c) {
     ...
     case StreamTokenizer.TT_EOL:
      if (result.m_props.size() > 0) {
       ...
       race.m_trackCond = result.m_trackCond;
       ...
       if (post != null) {
        if (result.m_track.trim().equals(post.m_track.trim())
         ...
         post.m_finishPos = result.m_props.getProperty("FINISHPOS");
         // Save odds and payoffs for horse
         post.m_odds = result.m_props.getProperty("ODDS");
         if (post.m_finishPos.equals("1")) {
          race.m_cloth1 = result.m_programNumber;
          race.m_win1 = result.m_props.getProperty("WINPAYOFF");
          race.m_place1 = result.m_props.getProperty("PLACEPAYOFF");
          race.m_show1 = result.m_props.getProperty("SHOWPAYOFF");
          post.m_winPayoff = result.m_props.getProperty("WINPAYOFF");
          post.m_placePayoff = result.m_props.getProperty("PLACEPAYOFF");
          post.m_showPayoff = result.m_props.getProperty("SHOWPAYOFF");
         } else if (post.m_finishPos.equals("2")) {
          race.m_cloth2 = result.m_programNumber;
          value = result.m_props.getProperty("WINPAYOFF");
          if (!result.m_props.getProperty("WINPAYOFF").equals(".00"))
           race.m_win2 = result.m_props.getProperty("WINPAYOFF");
          race.m_place2 = result.m_props.getProperty("PLACEPAYOFF");
          race.m_show2 = result.m_props.getProperty("SHOWPAYOFF");
          post.m_winPayoff = result.m_props.getProperty("WINPAYOFF");
          post.m_placePayoff = result.m_props.getProperty("PLACEPAYOFF");
          post.m_showPayoff = result.m_props.getProperty("SHOWPAYOFF");
          ...
          if (!result.m_props.getProperty("WINPAYOFF").equals(".00"))
           race.m_win3 = result.m_props.getProperty("WINPAYOFF");
          if (!result.m_props.getProperty("PLACEPAYOFF").equals(".00"))
           race.m_place3 = result.m_props.getProperty("PLACEPAYOFF");
          post.m_winPayoff = result.m_props.getProperty("WINPAYOFF");
          post.m_placePayoff = result.m_props.getProperty("PLACEPAYOFF");
          race.m_show3 = result.m_props.getProperty("SHOWPAYOFF");
          post.m_showPayoff = result.m_props.getProperty("SHOWPAYOFF");
   ...
   String wagerPayoff, String wagerBet)
 {
  ...
  if (result.m_props.getProperty(wagerType).equals("DAILY DOUBLE")) {
   payoffWork = Lib.atof(result.m_props.getProperty(wagerPayoff));
   // payoffBet = Lib.atof(result.m_props.getProperty(wagerBet));
