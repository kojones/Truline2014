...
public class Truline
{
 ...
 public static CommaDelimited bf           = new CommaDelimited();                            // bet correlations
 public static CommaDelimited tf           = new CommaDelimited();                            // trainer correlations
 ...
 {
  try {
   FileInputStream in = new FileInputStream("c:/truline2014/truline.ini");
  ...
  if (mode == GUIMODE && (file == null || !file.substring(0, 4).equals("LOAD"))) {
   ...
   ScratchFile.init(userProps, file);
   gui.init();
   try {
    startthread.join();
   } catch (Exception e) {
